...
ReducePrompt=(User1,User2,Num1,Num2,FinalAnsNr,FinalAnsDr)=>{
	let Nr=Number(User1);let Dr=Number(User2);
	getGCF=getGCF1x(User1,User2)
	checkNr=User1/getGCF;
	checkDr=User2/getGCF;
	if(Nr<=Num1 && Dr<=Num2 && getGCF1x(Nr,Dr)!=1 && checkNr==FinalAnsNr && checkDr==FinalAnsDr){
		return true;
	}else {
		return false;
	}
